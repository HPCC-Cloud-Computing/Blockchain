...
func (t *ChaincodeResult) getResultByTransactionID(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	queryString := fmt.Sprintf("{\"selector\":{\"result_transaction_id\":\"%s\"}}", resultTransactionID)
...
func (t *ChaincodeResult) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "getResultByTransactionID" {
		// get
		return t.getResultByTransactionID(stub, args)
	}
