...
func (t *CertificateChaincode) initCertificate(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	certificateTransactionID := args[0]
	...
	for _, value := range listResultTransaction {
		shim.Error("listResultTransaction:  " + value.ResultTransactionID)
...
func (t *CertificateChaincode) getCertificateByTransactionID(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	queryString := fmt.Sprintf("{\"selector\":{\"certificate_transaction_id\":\"%s\"}}", certificateTransactionID)
...
func (t *CertificateChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "getCertificateByTransactionID" {
		// create
		return t.getCertificateByTransactionID(stub, args)
	}
