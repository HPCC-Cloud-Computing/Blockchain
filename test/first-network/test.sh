...
peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n bbb3 -c '{"Args":["initProfile","3012","10A1,Ha Huy Tap,Kante,Mbappe,Toan$9.3&Ly$9.5,Kha,Hoc sinh gioi tinh","Tot nghiep cap 1#Tot nghiep cap 2"]}'
...
peer chaincode install -n users -v 3.0 -p github.com/chaincode/user01/go

peer chaincode upgrade -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n users -v 2.0 -c '{"Args":["init"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer')"